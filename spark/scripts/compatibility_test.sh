...
echo "val batch=\"1\""
echo "TestAutomationUtils.loadData(spark, \"${basePath}\" ,\"${tableName}\", conf=\"${conf}\", batch_id = batch, numInserts = 100, numUpdates = 10, numDeletes = 10)"
...
${SPARK_HOME}/bin/spark-shell --master local --driver-memory 8g \
--conf 'spark.serializer=org.apache.spark.serializer.KryoSerializer' --conf 'spark.sql.catalog.spark_catalog=org.apache.spark.sql.hudi.catalog.HoodieCatalog' --conf 'spark.sql.warehouse.dir=hdfs://localhost:8020/user/hive/warehouse' --conf 'spark.sql.extensions=org.apache.spark.sql.hudi.HoodieSparkSessionExtension' --conf 'spark.hadoop.spark.sql.legacy.parquet.nanosAsLong=false'  --conf 'spark.hadoop.spark.sql.parquet.binaryAsString=false' --conf 'spark.hadoop.spark.sql.parquet.int96AsTimestamp=true' --conf 'spark.hadoop.spark.sql.caseSensitive=false'  \
--packages org.apache.hudi:hudi-spark${spark_version}-bundle_2.12:${from_version}<< EOF
:load ../src/main/scala/com/hudi/spark/TestAutomationUtils.scala
val batch="1"
TestAutomationUtils.loadData(spark, "${basePath}" ,"${tableName}", conf="${conf}", batch_id = batch, numInserts = 100, numUpdates = 10, numDeletes = 10)
...
${SPARK_HOME}/bin/spark-shell --master local --conf 'spark.kryo.registrator=org.apache.spark.HoodieSparkKryoRegistrar' --driver-memory 8g \
--conf 'spark.serializer=org.apache.spark.serializer.KryoSerializer' --conf 'spark.sql.extensions=org.apache.spark.sql.hudi.HoodieSparkSessionExtension' --conf 'spark.sql.catalog.spark_catalog=org.apache.spark.sql.hudi.catalog.HoodieCatalog' \
--jars ${test_jar} << EOF
:load ../src/main/scala/com/hudi/spark/TestAutomationUtils.scala
...
echo "val batch=\"3\""
echo "TestAutomationUtils.loadData(spark, \"${basePath}\" ,\"${tableName}\", conf=\"${conf}\", batch_id = batch, numInserts = 100, numUpdates = 10, numDeletes = 0)"
echo "EOF"
...
val batch="3"
TestAutomationUtils.loadData(spark, "${basePath}" ,"${tableName}", conf="${conf}", batch_id = batch, numInserts = 100, numUpdates = 10, numDeletes = 0)
EOF
...
echo "val batch=\"5\""
echo "TestAutomationUtils.loadData(spark, \"${basePath}\" ,\"${tableName}\", conf=\"${conf}\", batch_id = batch, numInserts = 100, numUpdates = 100, numDeletes = 10)"
